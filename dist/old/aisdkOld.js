...
const deepResearch = async (prompt, depth = 3, breadth = 3) => {
...
export const aiToolKit = async (prompt) => {
...
const generateSearchQueries = async (query, n = 3) => {
...
const searchAndProcess = async (query, accumulatedSources) => {
                ...
                async execute() {
                    for (const pendingResult of [...pendingSearchResults]) {
                        ...
                        if (evaluation === "relevant") {
                            // avoid duplicates
                            if (!accumulatedSources.some((r) => r.url === pendingResult.url)) {
...
const generateLearnings = async (query, searchResult) => {
...
`;
const generateReport = async (research) => {
    const { text } = await generateText({
        model: google("gemini-2.5-flash"),
        system: SYSTEM_PROMPT,
        prompt: "Generate a comprehensive and sentiment-aware crime/security report based on the following research data:\n\n" +
...
// 	return text
// }
//# sourceMappingURL=aisdkOld.js.map