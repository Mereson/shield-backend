...
export const fetchArticleContentsWithExa = async (urls) => {
 ...
 */
const analyseArticlesBatch = async (articles, contentsByUrl, location) => {
    const analyses = [];
    const toSend = [];
    for (const article of articles) {
        ...
        if (!text || text.trim().length < 100) {
            // fallback minimal
            const pub = toISODate(content?.publishedDate) ??
            ...
            analyses.push({
                ...
                sentiment: "Neutral",
                confidence: 0.3,
                summary: "Insufficient content extracted; counted as neutral placeholder.",
 ...
 */
const aggregateMetrics = (deduped) => {
    // Distribution
    const distMap = {
        Positive: 0,
        Neutral: 0,
    ...
    const summary = `Coverage includes ${deduped.length} distinct crime events. Distribution: ` +
 ...
 */
const buildNarrativeReport = async (deduped, location) => {
    const head = deduped
 ...
 */
export const analyseNewsReport = async (generatedArticles, location) => {
