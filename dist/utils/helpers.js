 ...
 * --------------------
 */
export const fetchNews = async (query, from, to, maxItems = 20) => {
    try {
        ...
        if (response.status !== 200 || !response.data.articles) {
            throw createCustomError(`GNews API error: ${response.data.errors?.join(", ") || "Unknown error"}`, response.status || 500);
        }
