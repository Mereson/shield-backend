...
export const getSentiment = async (req, res, next) => {
    try {
        ...
        const formatLocationForNews = (location) => {
        ...
        const fetchedNews = await fetchNews(`(${searchLocation})`, previousDate, currentDate, 25);
        console.log(fetchedNews);
        const prompt = () => {
            if (fetchedNews.length > 0) {
                const articleReferences = fetchedNews
                    .map((article) => `- **${article.title}** ([${article.url}](@${article.url}))`)
                    .join("\n");
                return (`Sentiment and Urgency Analysis of Crime-Related Posts in ${searchLocation}, ` +
