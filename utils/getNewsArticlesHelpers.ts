...
import z from "zod"
import { Article, Result } from "../lib/types.js"
...
const normalizeUrl = (u: string): string | URL => {
	try {
		const x = new URL(u)
		x.hash = ""
...
const runToolSearch = async (query: string): Promise<Article[]> => {
...
const getRelevantArticles = async (
	...
	const { object } = await generateObject({
		...
		These are a list of news articles <search_result> ${JSON.stringify(
			toolResults
		)} </search_result>. 
