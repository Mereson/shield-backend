...
export const cleanNewsText = (
	text: string,
	type: "general" | "title" | "description" = "general"
): string => {
	...
	if (type === "title") {
		...
		cleaned = cleaned.replace(
			/^(breaking|urgent|update|exclusive|alert|news|report):\s*/gi,
			""
		)

		// Remove source attribution from titles
		cleaned = cleaned.replace(
			/\s*[-–—]\s*(reuters|ap|afp|cnn|bbc|fox|nbc|abc|cbs).*$/gi,
			""
		)
		...
		cleaned = cleaned.replace(
			/\b(read more|full story|continue reading).*$/gi,
			""
		)
